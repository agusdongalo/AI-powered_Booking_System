...
import { ArrowLeft, CalendarDays, Clock3, Plus, Scissors, Users, Sparkles } from 'lucide-react'
...
export function StaffDashboard() {
  ...
  const statCards = [
    {
      ...
      icon: CalendarDays,
      helper: format(selectedDate, 'MMMM d, yyyy'),
    },
    {
      label: 'Revenue today',
      value: `$${totalRevenue}`,
  ...
  return (
    <SiteShell
      ...
      description="A glassmorphism management view for bookings, services, schedules, and stylist operations."
      navHref="/staff"
      navLabel="Overview"
      backHref="/"
      accent="fuchsia"
                            ...
                            <div className="flex flex-wrap items-start justify-between gap-3">
                              <div>
                                ...
                                <p className="text-sm text-slate-500">
                                  {service?.duration} min • {service?.name}
                                </p>
                              </div>
                    ...
                    <tbody>
                      {services.map((service) => (
                        <tr key={service.id} className="border-b border-slate-100">
                          <td className="py-3 px-4 font-semibold text-slate-900">{service.name}</td>
                          <td className="py-3 px-4 text-slate-600">{service.duration} min</td>
                          <td className="py-3 px-4 text-slate-900">${service.price}</td>
